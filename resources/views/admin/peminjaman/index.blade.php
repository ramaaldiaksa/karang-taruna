...
<x-admin.toolbar :action="route('admin.peminjaman.index')">
    ...
    <x-admin.filter-select name="status" placeholder="Semua Status" :options="['menunggu' => 'Menunggu', 'disetujui' => 'Disetujui', 'ditolak' => 'Ditolak', 'dikembalikan' => 'Dikembalikan']" />
    <div></div>
</x-admin.toolbar>
                ...
                <tbody>
                    ...
                    <tr>
                        <td>
                            ...
                            <small class="admin-table__muted">Diajukan: {{ \Carbon\Carbon::parse($p->tanggal_pengajuan)->format('d M Y') }}</small>
                        </td>
...
@foreach($peminjamans as $p)
<x-admin.modal id="modalVerifikasi{{ $p->id_peminjaman }}" title="Detail Peminjaman & Verifikasi" icon="fas fa-info-circle" size="lg">
    <div class="modal-body admin-modal__body text-start">
        <div class="row mb-4">
            <div class="col-md-6 border-end">
                <h6 class="fw-bold text-secondary mb-3"><i class="fas fa-user me-2 text-primary"></i>Informasi Peminjam</h6>
            ...
            <div class="col-md-6 ps-md-4 mt-3 mt-md-0">
                <h6 class="fw-bold text-secondary mb-3"><i class="fas fa-calendar-alt me-2 text-primary"></i>Jadwal Peminjaman</h6>
                <table class="table table-sm table-borderless small mb-0">
        ...
        <h6 class="fw-bold text-secondary mb-3"><i class="fas fa-boxes me-2 text-primary"></i>Barang yang Dipinjam</h6>
        ...
        <div class="bg-light p-3 rounded-3 border">
            <h6 class="fw-bold text-secondary mb-3"><i class="fas fa-gavel me-2 text-primary"></i>Keputusan Verifikasi</h6>
            <form action="{{ route('admin.peminjaman.verifikasi', $p->id_peminjaman) }}" method="POST">
                ...
                <div class="d-flex gap-3">
                    <div class="form-check">
                        ...
                        <label class="form-check-label fw-semibold text-success ms-1" for="setuju{{ $p->id_peminjaman }}">Disetujui</label>
                    </div>
                    <div class="form-check">
                        <input class="form-check-input" type="radio" name="status" id="tolak{{ $p->id_peminjaman }}" value="ditolak">
                        <label class="form-check-label fw-semibold text-danger ms-1" for="tolak{{ $p->id_peminjaman }}">Ditolak</label>
                    </div>
                </div>
                <div class="mt-3 text-end">
                    <button type="submit" class="btn admin-modal__button admin-modal__button--primary"><i class="fas fa-save"></i><span>Simpan Keputusan</span></button>
                </div>
            </form>
        </div>
        @endif
